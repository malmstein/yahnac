...
public class StoriesParser {
    ...
    public Vector<ContentValues> parse(Story.TYPE type) {
        ...
        for (Element st : subtext) {
            ...
            String point_i = st.select("td[class=subtext]>span").text().replaceAll(" points", "").replaceAll(" point", "");
            String article_url_i = titles.get(i).select("td[class=title]>a").attr("href");
            String submitter_i = st.select("a[href*=user]").text();
            String comments_i = st.select("a[href*=item]").text().replaceAll("discuss", "0 comments");

            int item_id;
            try {
                String item_i = st.select("a[href*=item]").attr("href");
            ...
            String ago = parseAgo(st.text(), submitter_i);
    ...
    public static String parseAgo(String line, String submitter) {

        int start = line.indexOf(submitter) + 1;
